...
class top_block(gr.top_block):
    ...
    def __init__(self, sample_rate, frequency, freq_correction, rf_gain, if_gain, bb_gain, bandwidth, port):
        ...
        self.driverLime = True  # set driver flavour - gr-limesdr allows for better gain setting
        self.sample_rate = sample_rate
        self.rf_gain = rf_gain
        self.port = port
        self.if_gain = if_gain/10  # do not know why default values are overloading device and where do they come
        ...
        if self.driverLime:
            ...
            self.limesdr_source_0.set_bandwidth(bandwidth, 0)
            self.limesdr_source_0.set_digital_filter(sample_rate, 0)
            self.limesdr_source_0.set_gain(rf_gain, 0)
            self.limesdr_source_0.set_antenna(3, 0)  # LNAW for LimeSDR mini
            self.limesdr_source_0.calibrate(2.5e6, 0)

            self.connect((self.limesdr_source_0, 0), (self.blocks_tcp_server_sink_0, 0))
        else:  # driver osmosdr
            self.osmosdr_source_0 = osmosdr.source(
                args="numchan=" + str(1) + " " + 'driver=lime,soapy=0'
            )
            # self.osmosdr_source_0.set_time_unknown_pps(osmosdr.time_spec_t())  # disable it - causes ERROR
    ...
    def set_sample_rate(self, sample_rate):
        self.sample_rate = sample_rate
        if self.driverLime:
            self.limesdr_source_0.set_sample_rate(self.sample_rate)
    ...
    def set_bandwidth(self, bandwidth):
        self.bandwidth = bandwidth
        if self.driverLime:
            self.limesdr_source_0.set_bandwidth(self.bandwidth, 0)
        else:
